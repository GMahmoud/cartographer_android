...
//#include "cartographer_generic/test/parameter_block_test_ordering.h"
#include "ceres/parameter_block_ordering.h"
//#include "cartographer_generic/test/preprocessor_test.h"
//#include "ceres/preprocessor.h"

//#include "cartographer_generic/test/problem_test.h"
#include "ceres/problem.h"
//#include "cartographer_generic/test/problem_impl_test.h"
#include "ceres/problem_impl.h"
//#include "cartographer_generic/test/program_test.h"
...
bool CommonOptionsAreValid(const test::Options& options, string* error) {
...
bool TrustRegionOptionsAreValid(const test::Options& options, string* error) {
...
bool LineSearchOptionsAreValid(const test::Options& options, string* error) {
...
void StringifyOrdering(const vector<int>& ordering, string* report) {
  if (ordering.size() == 0) {
	  ::ceres::carto::StringAppendF(report, "AUTOMATIC");
    return;
  }

  for (int i = 0; i < ordering.size() - 1; ++i) {
	  ::ceres::carto::StringAppendF(report, "%d,", ordering[i]);
  }
  ::ceres::carto::StringAppendF(report, "%d", ordering.back());
}

void SummarizeGivenProgram(const Program& program,
...
void SummarizeReducedProgram(const Program& program,
                             test::Summary* summary) {
...
void PreSolveSummarize(const test::Options& options,
                       const ProblemImpl* problem,
                       test::Summary* summary) {

  SummarizeGivenProgram(problem->program(), summary);
  ::ceres::carto::OrderingToGroupSizes(options.linear_solver_ordering.get(),
                                 &(summary->linear_solver_ordering_given));
  ::ceres::carto::OrderingToGroupSizes(options.inner_iteration_ordering.get(),
...
//void PostSolveSummarize(const PreprocessedProblem& pp,
//                        test::Summary* summary) {
...
std::string SchurStructureToString(const int row_block_size,
  ...
  const std::string row =
      ...
      ? "d" : ::ceres::carto::StringPrintf("%d", row_block_size);

  const std::string e =
      (e_block_size == Eigen::Dynamic)
      ? "d" : ::ceres::carto::StringPrintf("%d", e_block_size);

  const std::string f =
      (f_block_size == Eigen::Dynamic)
      ? "d" : ::ceres::carto::StringPrintf("%d", f_block_size);

  return ::ceres::carto::StringPrintf("%s,%s,%s", row.c_str(), e.c_str(), f.c_str());
}

}  // namespace

bool test::Options::IsValid(std::string* error) const {
...
void test::Solve(const test::Options& options,
                   Problem* problem,
                   test::Summary* summary) {
  //using internal::PreprocessedProblem_test;
  //using internal::Preprocessor_test;
  ...
  *summary = Summary();
  ...
  Solver::Options modified_options = options.solver_option_;
  LOG(INFO) << "HERE 9.0";
  if (options.check_gradients) {
	LOG(INFO) << "HERE 9.1";
  ...
  LOG(INFO) << "HERE 9.2";
  //scoped_ptr<Preprocessor_test> preprocessor(
  //    Preprocessor_test::Create(modified_options.minimizer_type));
  LOG(INFO) << "HERE 9.3";
  //PreprocessedProblem_test pp;
  LOG(INFO) << "HERE 9.4";
...
using ceres::carto::StringAppendF;
using ceres::carto::StringPrintf;

std::string test::Summary::BriefReport() const {
...
std::string test::Summary::FullReport() const {
...
bool test::Summary::IsSolutionUsable() const {
