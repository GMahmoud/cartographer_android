...
void test::AddImuData(const int trajectory_id,
...
void test::AddTrajectoryNode(
		const int trajectory_id, const common::Time time,
		const transform::Rigid3d& point_cloud_pose) {
	CHECK_GE(trajectory_id, 0);
	node_data_.resize(
			std::max(node_data_.size(), static_cast<size_t>(trajectory_id) + 1));
	node_data_[trajectory_id].push_back(NodeDataTest{time, point_cloud_pose});
}

void test::AddSubmap(const int trajectory_id,
		const transform::Rigid3d& submap_pose) {
	CHECK_GE(trajectory_id, 0);
	submap_data_.resize(
			std::max(submap_data_.size(), static_cast<size_t>(trajectory_id) + 1));
	submap_data_[trajectory_id].push_back(SubmapDataTest{submap_pose});
}

void test::SetMaxNumIterations(const int32 max_num_iterations) {
	options_.mutable_ceres_solver_options()->set_max_num_iterations(
			max_num_iterations);
}

void test::Solve(const std::vector<Constraint>& constraints,
	...
	// TODO(hrapp): Move ceres data into SubmapDataTest.
	...
	for (size_t trajectory_id = 0; trajectory_id != node_data_.size();
			++trajectory_id) {
		TrajectoryDataTest& trajectory_data = trajectory_data_.at(trajectory_id);
...
const std::vector<std::vector<NodeDataTest>>& test::node_data()
const {
	return node_data_;
}

const std::vector<std::vector<SubmapDataTest>>& test::submap_data()
const {
	return submap_data_;
}
